...
{
				  // image im, int num, float thresh, box *boxes, float **probs, char **names, image *labels, int classes
	int i;
	struct BBox *head;
	struct BBox *p1,*p2;
	n = 0;
...
void detect_object(char *filename,network net,detection_layer layer,float thresh){
	...
	draw_detections_bbox(im, layer.side*layer.side*layer.n, thresh, boxes, probs, voc_names, 0, CLASSNUM);//建立结构体的动态链表
...
void run_yolo(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "go")) go_yolo(cfg, weights, filename,thresh);
}
