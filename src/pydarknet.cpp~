...
string voc_class_names[] = {"plate_license"};
int class_num = 1;
...
void draw_detections_bbox(image im, int num, float thresh, box *boxes, float **probs, string* names, image *labels, int classes, vector<_BBox> &bb)
{
	...
	for (i = 0; i < num; ++i){
		//probs:confidence数组
		//classes:分类
		//max_index():返回probs数组中i到classes范围内最大值下标
		//cout<<classes<<endl;
		int classs = max_index(probs[i], classes);//classs:最大confidence值的下标
		//printf("\nclasss:%d \n",classs);

		//int len=sizeof(probs)/sizeof(float);
    		//int   n=sizeof(probs[0])/sizeof(float);
  		//int   m=len/n;
		//cout<<m<<endl<<n<<endl;//m=1,n=2;
...
public:
	...
	bp::list detect_object(bp::str img_data, int img_width, int img_height, int img_channel){
		...
		vector<_BBox> bboxes;//存放所有bounding box.
		...
		draw_detections_bbox(im, layer.side*layer.side*layer.n, thresh, boxes, probs, voc_class_names, 0, class_num, bboxes);//搜索class_num个类,来构建bboxes以及标注出结果框,
