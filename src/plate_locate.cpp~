...
//此情况下不试用：大角度，而且切割的很好，原因是findContours，无法准确找出相当于填充满的这个正确contours。
...
// 进行颜色查找
  colorMatch(src, match_grey, r, false);// 输出灰度图
...
//问题出在findContours()身上，
//findContours(), Finds contours(轮廓) in a binary image.
  ...
  while (itc != contours.end()) {
    RotatedRect mr = minAreaRect(Mat(*itc));//RotatedRect minAreaRect(InputArray points),获取RotateRect的最大外围正矩形，返回RotatedRect
						//Parameters:	points –Input vector of 2D points, stored in: std::vector<> or Mat (C++ interface)
...
/*
	cout<<"colorSearch::outRects size:"<<outRects.size()<<endl;
	
	for(int i=0;i<outRects.size();i++){
		Point2f vertices[4];
		outRects.at(i).points(vertices);
		for (int i = 0; i < 4; i++)
		    line(match_grey, vertices[i], vertices[(i+1)%4], Scalar(0,255,0));

		Rect brect = outRects.at(i).boundingRect();
		rectangle(match_grey, brect, Scalar(255,0,0),1,8,0);

		imshow("rotatedRect:", match_grey);
		waitKey(0);
	}
	*/
...
int CPlateLocate::deskew(const Mat &src, const Mat &src_b,
  ...
  for (size_t i = 0; i < inRects.size(); i++) {
    ...
    float r = (float) roi_rect.size.width / (float) roi_rect.size.height;//RotatedRect的宽/高
    float roi_angle = roi_rect.angle;//RotatedRect的角度
	//cout<<"deskew:r："<<r<<" roi_rect.size.width："<<roi_rect.size.width<<" roi_rect.size.height："<<roi_rect.size.height<<endl;

    Size roi_rect_size = roi_rect.size;//size – Width and height of the rectangle.
    if (r < 1) {//如果这个RotatedRect不是横着放的， 
      roi_angle = 90 + roi_angle;
      swap(roi_rect_size.width, roi_rect_size.height);//swap(),元素交换
    }

    if (m_debug) {
      Point2f rect_points[4];//定义四个点坐标
	...
	//std::cout << "deskew::roi_angle:" << roi_angle << std::endl;
    ...
    if (roi_angle - m_angle < 0 && roi_angle + m_angle > 0) {
      ...
      if ((roi_angle - 5 < 0 && roi_angle + 5 > 0) || 90.0 == roi_angle ||
        ...
        // we need affine for rotatioed image,如果图片偏斜，还需要视角转换 affine
...
int CPlateLocate::plateColorLocate(Mat src, vector<CPlate> &candPlates,
                                   int index) {
cout<<"plateColorLocate"<<endl;
  vector<RotatedRect> rects_color_blue;
  rects_color_blue.reserve(64);//给vector预分配存储区大小，即capacity的值 ，但是没有给这段内存进行初始化
...
#pragma omp section
    {
      ...
      deskew(src, src_b_blue, rects_color_blue, plates_blue, true, BLUE);//! 抗扭斜处理
    }
#pragma omp section
    {
